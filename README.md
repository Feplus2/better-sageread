...
| 🎯 **自定义技能** | 创建个性化 AI 提示词模板，打造专属阅读工作流；兼容导入 SKILL.md 文件（Claude Code skills 生态） | ✅ |
| 🔌 **MCP 扩展** | 通过 MCP 协议接入外部工具：远程 HTTP/SSE 与本地 stdio（npx/uvx）双传输，市场一键安装，Agent 可自管理 | ✅ |
| 🔑 **密钥保管箱** | API Key 统一存入系统凭据管理器，配置中用 `{{secret:名称}}` 引用，永不明文落盘与回显 | ✅ |
