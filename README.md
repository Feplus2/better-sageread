...
- **思考强度五档**：推理力度按需调节（聊天模型选档、辅助模型自动最低档）；关不掉思考的型号（GLM-5.3、Gemini 3 等）自动适配
- **双通道搜索**：联网搜索（实时资讯/外部背景）+ Sciverse 科研证据检索（直回论文原文片段，带标题/页码/坐标），三个助手都会用
...
| 书籍翻译 | 无 | 全书对照翻译（三态显示/断点续翻/切句器与句词对齐/hover 联动/标注镜像） |
| AI 体系 | 单助手简单问答 | 三助手分工 + 全局 Agent（导入/整理/转换/设置全能代做）+ 思考强度五档调节 + 技能 + MCP + 密钥保管箱 + 用量统计 |
| 搜索 | 完全不具备 | 联网搜索 + Sciverse 科研证据检索（直回论文原文片段，带标题/页码/偏移坐标） |
| 书库检索 | 有（当前书的向量化 RAG） | 书籍 RAG 沿用并增强 + 论文库跨论文语义检索（主题调研/按方法找文献），AI 回答带原文证据 |
| Agent 工程 | 无对话压缩与工具目录概念，长会话 token 消耗大；Agent 不能读写本地文件；用户无法预览 Agent 产物 | 上下文活塞滚动压缩（256k/128k 双水位 + 滚动摘要）+ 工具目录牌按需取说明书（工具多时 schema 不常驻提示词）+ 文件五件套落盘 Agent 工作区 + Agent 产物侧边栏实时预览 |
| 笔记与对话 | 不支持管理与导出 | 长文 Markdown 笔记面板（AI 可直接落笔）+ 划线想法导出 + 对话标星/改名/删除/导出（Markdown/HTML/PNG） |
| 任务管理 | 无 | 统一任务中心（队列/并发/进度卡/刷新恢复） |
| 数据安全 | 无 | WebDAV 完整备份 + 多设备增量同步 + 密钥入系统凭据管理器 |
| 标签页 | 单横排、无休眠 | 垂直/横排切换、不常用标签自动休眠保活、侧栏位置互换 |
