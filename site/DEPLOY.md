...
1. **下载流量接入 EdgeOne**：EdgeOne 控制台给站点添加加速域名（如
   `dl.bettersageread.cn`），回源类型选 COS、源站填 COS 桶默认域名。
   然后把 `config.js` 的 `cosBase` 改为 `https://dl.bettersageread.cn`。
   （Pages 托管的网页本体不受影响；加速域名的缓存对安装包这类大文件还有提速收益。）
2. **频次控制（防反复烧）**：EdgeOne 控制台 → 站点 → 安全防护 → Web 防护 →
   「CC 攻击防护 / 频次控制」新建规则：
   - 匹配：URL 路径包含安装包后缀（`.exe` / `.zip`，或直接全站）
   - 统计维度：客户端 IP
   - 阈值：**每 IP 每天 5 次**（正常用户一生只下一两次；应用内更新走 GitHub
     通道不经本站，不受此限）
   - 触发动作：拦截（建议先「JS 挑战」观察一周，误判率低再改「拦截」），
     拦截时长 1 小时起步
3. **用量封顶（二次止损）**：EdgeOne → 用量管理 → 带宽/流量封顶规则，
   如日流量超 5GB 自动暂停加速（回源切直连或下线，按你能接受的口径选）。
4. 设备维度（同设备换 IP 反复烧）免费版做不到精确识别，IP+UA 组合规则可近似；
   真有规模攻击再考虑 Bot 管理（付费）。
