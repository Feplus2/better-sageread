...
- **公式重复 \tag 编译错误根修（2026-08-14，Papers_Converter 工作区未提交，exe 已重打同步双实例 binaries）**：cosmic strings 论文（arXiv 2605.22944）Eq.(50)/Eq.(A2) 实测——两行式 array 公式的编号 "(50)" 沿行竖排拆开时，MinerU VLM 先给首行残缺编号生成 `\tag {5}`、末尾又补完整 `\tag{50}`，双 `\tag` 被 KaTeX 拒收（"Multiple \tag"）。converter 侧 `_dedup_equation_tags`（content_processor `_build_ir` 公式分支调用）：同块多 `\tag` 只留一个，保留内容最长者、并列取最末；单测 `test_equation_tags.py` 8 例全绿。存量数据已修复（全库扫描仅此一篇 4 份 paper.md 中招：两 book 副本 + 两 converter 产物）；应用内 CDP 滚动采集验证全文 60 条编号公式渲染、`.katex-error` 0 个
