...
>
> **阶段状态（2026-07-27）**：基础协议已验证可用（全量引导、推送、拉取、应用、资产通道），实时性打磨（请求节流、断网重试优化）定位为移动端铺路，暂缓。
...
- **新设备开荒**：配好 WebDAV 开增量同步，等 1-2 分钟图书馆自动满员（每轮同步设备都会登记进云端 devices.json，老设备发现新设备即全量引导；**设备 ID 是本机自己的**，`sync-state.json` 不进备份包）
...
- **坚果云 503 限流**：两个实例各 25 秒一轮、每轮十几个请求，很容易触发坚果云免费账户限流（HTTP 503）。503 会阻断整轮同步，但下一轮自动恢复。本机双实例测试时建议拉取频率调为 5 分钟，或接受偏尔 503。单用户多设备（真实场景）请求量远低于此，不是问题
- ~~**同步轮中途失败导致状态回退**~~ **已修复（2026-07-27）**：原为 pull 失败时 record_l2_failure 从磁盘读回旧状态覆写，导致 push 水位和失败包计数被回退（3-strike 永远无法跳过坏包）。现在 push 成功后立即持久化水位，pull 失败时先保存已有进度再报错
- ~~**元数据首次全量引导未实现**~~ **已修复（2026-07-26）**：首次同步自动把 8 张同步表存量全量回填进 changeset（`bootstrapped_at`）；发现新设备加入且本地有书时再回填一次（`bootstrap_peers`）；应用侧 FK 延迟到提交校验，整包不再因“父行后至”被静默丢弃。**新设备无需再“恢复”备份开荒，开增量同步即可收到存量书**。真机复验又暴露两个引导链路漏洞，同日一并修复：① 新设备无变更可推时永不登记 devices.json（老设备发现不到它）→ 现在**每轮同步（含纯拉取）必登记**；② 引导包之前的乱序包应用失败被“整包跳过+水位照推”永久丢弃 → 现在**失败包不推水位、下轮重试、满 3 次才跳过**
- B 端若无任何推送，不会出现在云端 `devices.json` 索引里（只有指针文件），属正常现象
- **幽灵设备**：反复重置 B 实例会在云端 devices.json 残留旧 device_id，新 B 会浪费几轮拉取它们的坏包（FK 失败，3 轮后自动跳过，不影响正常同步）。彻底清理需手动删除云端 `sageread/sync/` 目录重建
- **云端目录搬家与回退**（2026-07-27）：云端布局统一为 `sageread/{sync,backups}`，启动时自动用 WebDAV MOVE 搬家（幂等、有哨兵）。**坚果云对集合 MOVE 返回 403**——此时自动回退：L2 在 `webdav-config.json` 写入 `l2_root` 覆盖继续用 `sageread-sync`，备份保持旧 `remote_dir`，**同步/备份都不会断**。想要新布局：在坚果云网页端手动把两个目录移进 `sageread/`（网页端不受 WebDAV MOVE 限制），下次启动自动识别并清除覆盖；若 `sageread/sync` 是搬家事故留下的空壳，确认后直接删掉
...
- `发现新设备 …，已生成全量引导（N 行），将于下一轮推送` = 老设备引导触发正常
- `收到书籍元数据 N 条` = 新设备引导到达
- `应用 changeset 失败，不推水位，下轮重试` = 乱序/坏包，自动重试（满 3 次才跳过并告警）
- `changeset 解压失败（疑似半截包），不推水位，下轮重试` = 传输性半截包（PUT 非原子），自动重试（满 40 轮≈20 分钟才跳过；2026-08-26 P3 起不再计入 3 次上限）
- `应用 changeset 失败满 3 次` = 永久坏包，要查
...
| 现象 | 原因 | 处置 |
|---|---|---|
| 新设备书架空 | B 未登记进 devices.json（旧版只在推送时登记，已修复）或引导包在路上 | 等 1-2 分钟；查云端 devices.json 有无 B、A 端日志"发现新设备…已生成全量引导"、B 端日志"收到书籍元数据 N 条" |
| 日志反复"应用 changeset 失败，不推水位，下轮重试" | 乱序包（父行未到）或坏包 | 无需处置：不推水位下轮自动重试，引导包到达即恢复；满 3 次才跳过并告警 |
...
| 进度不跳 | 停留不足 30 秒/60 秒防跳动窗口内 | 看 toast；点顶栏全局同步按钮强制对齐 |
| 书到了但打不开 | 文件未下载完或 sha 不符 | 重开该书触发重下 |
| 两端同步全部卡住 | 坚果云 503 限流（双实例 25s 轮询请求量过大） | 等 1-2 分钟自动恢复；或拉取频率调为 5 分钟 |
| 日志反复“读取设备索引失败，本轮跳过拉取” | 坚果云 503/网络异常（已修复静默吞错，现在有明确日志） | 同上 |
