...
> 2026-08-25 建立。前置调研：`docs/archive/task-system-survey.md` §6（并发可行性，含资源画像与锁分析）。
> **强依赖 P2**（`docs/archive/task-queue-p2-plan.md`）：通道化完成后并发才有意义；本文档各阶段按 P2 落地后的代码位置描述。
>
> **状态（2026-08-25）：已全部实施并实盘验证通过**（commit 8818972）——解析并发 2 实盘观测
> 最大并发恰 2、第三篇槽满排队、全部成功落库；向量化并行 embed 实盘成功且版本锚一致；
> 刷新恢复 E2E（情形 B 冻结窗口）PASS；pending_done 多槽持久化含旧版迁移。
