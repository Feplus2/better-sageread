...
> 本文档记录我们（fork 方）的定制计划，**不提交给上游**。最后更新：2026-07-28。
...
- **2026-07-28 状态更新**：网络搜索验证通过、备份轮转+管理入口已做、Books_Converter 端到端已验证、格式策略收窄已做（保留其他格式为有意兼容）、PDF 阅读体验修复已验证、非 EPUB 降级提示已做、向量化非 EPUB 报错已做、整理划线导出已做（exportNotes 工具）。**L2 增量同步标记 BETA**（移动端前不投入深度加固）。**第四批大件全部完成。**
...
### ~~网络搜索失败~~（✅ 2026-07-28 验证通过）
- 非敏感查询验证通过，搜索功能可用。敏感查询仍受审查限制（必应/百度），DuckDuckGo+代理可绕过。
...
### ~~同步遇 503 无退避~~（降级为 BETA 已知限制）
- L2 增量同步整体标记为 **BETA**（见下方说明）。503 退避留待移动端等高频场景开发时再完善。

### ~~备份无保留上限与管理入口~~（✅ 2026-07-28 已完成）
- 备份轮转保留策略 + 管理入口已实现。

### tags 跨设备同名 UNIQUE 冲突（skills 同款，2026-07-27 记账）

- skills 已修（按名合并，e06dc2c）；`tags.name` 同样 UNIQUE，两台设备各建同名标签会撞 2067。无默认数据、概率低，待把 name 匹配下沉为注册表级 unique-key 兜底时一并处理。

### L2 增量同步标记为 BETA（2026-07-28 决策）

- **理由**：L2 高频使用场景为多设备（尤其移动端），当前无移动端开发计划，桌面双设备使用频率低。投入大量精力做详尽测试和边缘加固（503 退避、存量回填等）意义有限。
- **现状**：核心功能可用（元数据/书籍文件/资产通道均已实现），设置页标注 BETA。
- **待移动端立项时再完善**：503/429 指数退避、存量回填引导、高频压力测试。
...
| | 阅读助手 | 全局助手 |
|---|---|---|
| 入口 | 书内侧边栏对话（绑定 book_id） | 主页“全局助手”（book_id=undefined） |
| 范围 | 当前这本书 | 整个书库 / 全局 |
...
**关键设计原则：动作工具只赋予全局助手。**
- 整理标签、批量向量化、备份、同步、导入导出等**全局性动作工具**，只挂给全局助手。
- **不**把这些动作工具塞进阅读助手——避免工具过多导致模型选择混乱、误触发，保持阅读助手“专注本书问答”的精简工具集。
- 阅读助手保持只读 + 本书 RAG；全局助手才是“干活”的入口。

**落地路径（渐进，不换架构）**：现有工具循环天然支持多步任务，只需把现有 GUI 功能封装成**动作工具**挂给全局助手：
...
- ~~**候选：APP 帮助助手**~~（✅ 已完成 2026-07-27）：内置使用手册（`plugins/tauri-plugin-epub/resources/manual/*.md`，编译期嵌入）落成 `books/__app_manual__` 虚拟语料库，复用书籍同款 chunk/embed/search 管线（`plugin:epub|index_manual` + `search_db`）；全局助手工具 `askAppHelp`，有向量能力走语义检索、无则降级本地关键词检索；内容哈希驱动自动重建
- ~~**待办：ima 侧 MCP 配置**~~（标记 BETA 2026-07-28）：社区 tencent-ima-copilot-mcp 或官方 API，暂无明确应用场景，待有需求时再验证
...
- **会话列表的多选管理**（2026-07-22）：对话列表页加多选（区别于全局助手页内的多选导出）：多选删除、多选星标/取消星标、多选导出
...
- **预览面板**（2026-07-22）：AI 产物（解读报告、思维导图 HTML、分析长文）需要"写得出 + 看得见"——侧边栏拉一个预览窗口直接阅览 HTML/MD 文件，别让用户去系统文件管理器里找。书与文献通用，高频功能。**2026-08-06 已纳入 `docs/archive/agent-ecosystem-plan.md` E5 批**
- **一句话 Zotero 导入**（2026-07-22，详见 `docs/archive/paper-reading-feasibility.md` 阶段 3 补充）：对话面板挂 zotero-brain MCP，文献连同 Zotero 分类文件夹导入，collection 层级映射为 SageRead 标签/分组
