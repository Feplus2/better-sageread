...
- ~~**主题 CSS 上限探索（低优先，视觉向）**：图片/视频为底 + 毛玻璃等示例，探索全局主题系统能力边界（THEMING.md）；排在技术优化之后~~（✅ 2026-08-05 部分消化：主题「怜烟」落地——背景视频循环（亮=哀鸿水墨 0.7MB / 暗=苏怜烟暖金 1.0MB，1080p30 H.264 faststart）+ 变量半透明化 + 主承力面 backdrop-filter 毛玻璃；视频层 `components/theme-background-video.tsx` 读 CSS 变量 `--bg-video`（:root/html.dark 分流）挂载，`main.tsx` 全局挂载；登记 BUILTIN_THEMES。配色：亮色宣纸白×墨黑×鸦金，暗色紫檀黑×昏金×淡紫）
