...
### AI 用量统计面板（2026-08-26 用户点子，锦上添花）

主页「阅读统计」目前只有阅读数据；我们同时也是 AI 应用，可以加一块 AI 用量统计：
- token 用量（输入/输出/缓存命中分列）、模型使用占比；
- 时间跨度：日/周/月/季度，折线图（趋势）、饼图（模型占比）、柱状图（日用量）等；
- 价格映射表：各模型 输入/输出/缓存 单价维护一张表，token → ¥ 直观换算；
