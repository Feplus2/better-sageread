...
6. **论文 + L2 资产通道缺口（2026-08-10 实证）**：`importPapers` 链路不把论文文件推入 L2 资产通道，对端只收到 books 元数据行——书库出现卡片但点击报"论文文件读取失败"。需补：① 论文导入后的文件上传联动；② 论文卡片的"仅云端→点击下载"流（书籍有，论文疑似没有）；③ `images/` 不在文件通道是历史已知限制，paper.md 体系（paper.md+images/ 多文件）与单文件书籍通道的适配要一并设计。另外 L2 同步前安全快照偶发 `table threads already exists`（快照失败仍继续同步=该轮无回滚保险），一并排查
