...
- 编辑态：点开卡片 → **在左侧边栏区域内**进入编辑/预览子视图（像点进子文件夹），**不扩宽侧栏**——标题行 + Markdown textarea（等宽字体，已挂标点自动配对）+ **编辑|预览 同一板块切换**（Typora 式，非上下堆叠；预览走 react-markdown + KaTeX + 当前主题）；位置 tag 可手输、可从 **TOC 选择器**选章节（同时写入精确锚点）；自动保存（debounce）+ 关闭未保存提示
