# Better SageRead 全局主题开发指南

Better SageRead 支持 Typora 式的全局主题：把一个 `.css` 文件放进主题文件夹，在设置里选中，整个应用界面就会换肤。本文面向主题作者，介绍主题机制、可用变量与语义钩子。

## 两层配色架构

Better SageRead 有两套互不影响的配色系统：

| 层 | 控制对象 | 配置方式 |
| --- | --- | --- |
| 应用 UI（本系统） | 书架、导航、侧边栏、AI 问答区、弹层、设置页 | 全局主题 `.css` 文件（本文档） |
| 书籍内部 | EPUB 正文、文字与背景色 | 阅读页设置里的"配色"（文字颜色、纯色/照片/自定义背景与遮罩） |
...
1. 找到用户主题文件夹（不存在会自动创建）：
   - Windows 正式版：`%APPDATA%\com.bettersageread.app\themes`
   - Windows 开发版（identifier 带 `.dev` 后缀）：`%APPDATA%\com.bettersageread.dev\themes`
...
**内置主题不在主题文件夹里**：内置主题（羊皮纸/蛋糕/怜烟/蒙德里安/莫兰迪）随应用打包、只读，文件夹里看不到它们是正常的。想基于某个内置主题微调（比如怜烟的透明度参数），从仓库 `public/themes/` 复制对应 css 源码出来，同名（覆盖）或改名（并存）后放入主题文件夹再改。
...
主题可以声明一条循环视频作为全局动态壁纸（参考内置主题 `lianyan.css`，文件头部有完整的透明度调参注释）：

```css
:root {
  --bg-video: url("/themes/我的主题/light.mp4"); /* 亮色模式视频 */
  --bg-frost: blur(10px) saturate(1.15);        /* 壁纸上方的毛玻璃 */
}
html.dark {
  --bg-video: url("/themes/我的主题/dark.mp4");  /* 暗色模式视频 */
}
```

- 声明了 `--bg-video` 的主题，应用会自动挂载全屏循环视频层；不声明则完全没有此开销
- 视频文件随主题放进 `public/themes/<主题名>/` 目录（内置主题的做法）；用户主题如需自带视频，目前需使用可访问的绝对 URL
- 此时各层背景变量（`--background`、`--card`、`--popover`、`--sidebar` 等）应使用 rgba 半透明色，壁纸才透得出来
- **阅读区联动**：视频壁纸主题下，阅读器选纯色"默认"背景会自动半透明化透出壁纸；浓度由 `--reader-solid-alpha`（0~1，缺省浅色 0.70 / 深色 0.82）控制
