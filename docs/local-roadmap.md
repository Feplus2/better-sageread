...
2. **第四批·大件（各 2-3 天）**：~~Books_Converter 整合（PDF→EPUB→入库，复用辅助模型）~~（已完成 2026-07-26，真机端到端待测）、~~标签栏纵横切换~~（已完成）、APP 帮助助手、**全局助手动作工具**（向量化/整理划线/整理标签/备份/同步，见《Agent 架构设想》）
...
### 备份无保留上限与管理入口（2026-07-27 用户提）

- **现象**：自动备份开着会无限堆积在坚果云（同步前快照有"留 3 份"机制，备份却没有）；GUI 无"最多保留 N 份"设置或手动管理（查看/删除）入口。
- **待做**：备份轮转保留策略（默认留最近 10 份，可配）+ 恢复面板支持删除旧备份。
