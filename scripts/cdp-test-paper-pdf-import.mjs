// E2E：通过 CDP(9223) + vite 模块注入，在运行中的 SageRead 里走真实 startPaperPdfImport 流程
// 验证：store 读参 → convert_paper_pdf spawn → 事件流 → done → scan_papers_dir 可扫描（不入库）
// 运行：node scripts/cdp-test-paper-pdf-import.mjs（需 dev 实例以 WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port=9223 启动）
import { readFileSync } from "node:fs";

const LIST_URL = "http://127.0.0.1:9223/json/list";
const PDF = String.raw`F:\MyProjects\zotero-brain\parsed\2EL339RU\2EL339RU.pdf`;
...
(async () => {
  ...
  // token 来自用户自己的 Papers_Converter/.env；已配置则不动
  if (store.paperEngine !== "paddleocr") store.setPaperEngine("paddleocr");
  if (!store.paddleocrToken) store.setPaddleocrToken(${JSON.stringify(env.PADDLEOCR_TOKEN ?? "")});
  const tokenError = svc.paperEngineTokenError("paddleocr");
...
if (value.scan) console.log("扫描产物:", JSON.stringify(value.scan).slice(0, 300));
ws.close();
if (value.terminal?.type === "done" && value.scan && !value.scan.error && value.scan.length === 1) {
  console.log("✓ E2E 通过：startPaperPdfImport 真实流程走通（spawn+协议+产物可入库）");
