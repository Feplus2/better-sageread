// CDP 真机验证（jieba 版）：ZH 划 "尖晶石" → EN 锚点应为 "spinel"（单字版会拖出 rocksalt）
...
const coords = JSON.parse(await evalJs(`(() => {
  ...
  let node; while ((node = walker.nextNode())) {
    if (node.parentElement.closest('[data-translation]') && node.textContent.includes('尖晶石')) break;
  }
  if (!node) return 'no-node';
  node.parentElement.scrollIntoView({ block: 'center' });
  const i = node.textContent.indexOf('尖晶石');
  const r1 = document.createRange(); r1.setStart(node, i); r1.setEnd(node, i + 1);
  const r2 = document.createRange(); r2.setStart(node, i + 2); r2.setEnd(node, i + 3);
...
if (typeof coords !== "object") { console.log("定位失败:", coords); ws.close(); process.exit(1); }
...
async function drag(x1, y1, x2, y2) {
  ...
  for (let i = 1; i <= 10; i++) {
    await call("Input.dispatchMouseEvent", { type: "mouseMoved", x: x1 + ((x2 - x1) * i) / 10, y: y1, button: "left", buttons: 1 });
...
for (let attempt = 1; attempt <= 3 && !ok; attempt++) {
  ...
  console.log(`尝试 ${attempt}: 选区=${JSON.stringify(sel)}`);
  ok = sel === "尖晶石";
  if (!ok) await evalJs(`window.getSelection()?.removeAllRanges()`);
}
...
console.log("清理:", await evalJs(`(async () => {
  ...
  const targets = notes.filter(n => n.text === 'spinel');
